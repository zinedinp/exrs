...
// `image`/`png` crates. `png` hardcodes flate2's default features
// (`miniz_oxide`) with no opt-out, so depending on it links a second,
// never-called zlib backend into every dev/test/bench/example build.
// Scope is deliberately narrow: 8-bit grayscale/RGBA encode, and just enough
// decode (non-interlaced 16-bit truecolor RGB) to read the one fixed
// ground-truth PNG used by `tests/across_compression.rs`.
