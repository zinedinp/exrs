...
//! functions:
//!
//! 1. `read_first_rgba_layer_from_file(path, your_constructor,
//!    your_pixel_setter)`: You specify how to store the pixels. The first layer
//!    containing rgba channels is then loaded from the file. Fails if no rgba
//!    layer can be found.
//!
//! 1. `read_all_rgba_layers_from_file(path, your_constructor,
//!    your_pixel_setter)`: You specify how to store the pixels. All layers
//!    containing rgba channels are then loaded from the file. Fails if any
//!    layer in the image does not contain rgba channels.
//!
//! 1. `read_first_flat_layer_from_file(path)`: The first layer containing
//!    non-deep data with arbitrary channels is loaded from the file. Fails if
//!    no non-deep layer can be found.
//!
//! 1. `read_all_flat_layers_from_file(path)`: All layers containing non-deep
//!    data with arbitrary channels are loaded from the file. Fails if any layer
//!    in the image contains deep data.
//!
//! 1. `read_all_data_from_file(path)`: All layers with arbitrary channels and
//!    all resolution levels are extracted from the file.
//!
...
// 1. `ReadImage` - The specification. Contains everything the user wants to
//    tell us about loading an image. The data in this structure will be
//    instantiated and might be borrowed.
// 2. `ImageReader` - The temporary reader. Based on the specification of the
//    blueprint, a reader is instantiated, once for each layer. This data
//    structure accumulates the image data from the file. It also owns temporary
//    data and references the blueprint.
// 3. `Image` - The clean image. The accumulated data from the Reader is
//    converted to the clean image structure, without temporary data.
...
/// The first closure creates an image, and the second closure inserts a single
/// pixel. The type of the pixel can be defined by the second closure;
...
where
    ...
    B: FromNativeSample,
    A: FromNativeSample,
    Create: Fn(Vec2<usize>, &RgbaChannels) -> Pixels, /* TODO type alias?
                                                       * CreateRgbaPixels<Pixels=Pixels>, */
...
/// Utilizes the builder pattern to configure an image reader. This is the
/// initial struct.
...
impl ReadBuilder {
    ...
    // e. g. `let floats = reader.any_channels_with(|sample, f32_samples|
    // f32_samples[index] = sample as f32)` pub fn no_deep_data_with <S> (self,
    // storage: S) -> FlatSamplesWith<S> {  }
