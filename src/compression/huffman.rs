...
//
// - `dual_lut` (x86/x86_64, no `huffman-lut-*` feature pinned): both a
//   13-bit and a 14-bit decoder are compiled in (two monomorphized structs,
//   generated by the same macro from a literal `$lut_bits`, so each keeps
//   `LUT_BITS` a true compile-time constant and LLVM can still prove
//   `prefix < lut_symbol.len()` from the shift alone.
//   A single dynamically-sized
