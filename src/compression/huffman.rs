...
// Leading buffer bits resolved with a single table lookup ("LUT_BITS" below);
// keeps the two-table (`lut_symbol` + `lut_length`) footprint L1-resident.
// Bigger means fewer codes fall through to the `resolve_long_code`
// linear-scan fallback, at the cost of a bigger per-chunk table rebuild and
// more L1d pressure
//
// - 12 bits = 4096 * 5 bytes = 20 KiB
// - 13 bits = 8192 * 5 bytes = 40 KiB
// - 14 bits = 16384 * 5 bytes = 80 KiB
//
// The tier is picked in one of two ways, gated by the modules below:
//
// - `dual_lut` (x86/x86_64, no `huffman-lut-*` feature pinned): both a 13-bit
//   and a 14-bit decoder are compiled in (two monomorphized structs, generated
//   by the same macro from a literal `$lut_bits`, so each keeps `LUT_BITS` a
//   true compile-time constant and LLVM can still prove `prefix <
//   lut_symbol.len()` from the shift alone. A single dynamically-sized
//   `Vec`-backed table with a runtime `lut_bits` field was tried first and
...
//   0x8000_0005 on older AMD parts) picks 14 bits if the detected L1d is >=96
//   KiB (the raw 80 KiB two-table footprint plus ~16 KiB headroom for the rest
//   of the decoder's per-iteration working set. Hybrid P/E parts (leaf 7 EDX
//   bit 15) always get 13 bits, since the probe only sees one core's L1d.
...
//   `huffman-lut-*` feature pinned): a single compile-time `LUT_BITS`, same as
//   before this runtime-dispatch addition -- a pinned feature wins outright;
//   otherwise Apple Silicon (`cfg!(target_os = "macos", target_arch =
//   "aarch64")`) defaults to 14 bits (unverified, same reasoning as above, just
//   resolved at compile time instead of runtime since `raw-cpuid` only speaks
//   x86 CPUID and no aarch64 hardware was available to build or verify an
//   equivalent probe), everything else defaults to 13.
...
compile_error!("huffman-lut-12 and huffman-lut-14 are mutually exclusive");
...
mod fixed_lut {
    ...
    #[cfg(feature = "huffman-lut-12")]
    const LUT_BITS: usize = 12;
    #[cfg(feature = "huffman-lut-13")]
    ...
    pub(super) struct CanonicalDecoder {
        max_len: usize,
...
/// Runtime dispatch between two compile-time-monomorphized decoders (13-bit
/// and 14-bit `LUT_BITS`); see the doc comment above. Active on x86/x86_64
/// when no `huffman-lut-*` feature is pinned.
#[cfg(all(
    any(target_arch = "x86", target_arch = "x86_64"),
    not(any(feature = "huffman-lut-12", feature = "huffman-lut-13", feature = "huffman-lut-14"))
))]
mod dual_lut {
    ...
    /// Minimum detected L1d size (bytes) before switching to the 14-bit LUT
    /// tier: the raw 80 KiB two-table footprint plus ~16 KiB headroom for
    /// the rest of the decoder's per-iteration working set. Not itself
    ...
    /// Generates a `CanonicalDecoder` monomorphized for a literal
    /// `$lut_bits`, so `LUT_BITS` stays a true compile-time constant inside
    /// (see the doc comment above `MAX_CODE_LENGTH` for why that matters).
    /// Identical logic to the single-tier version in `fixed_lut`, just with
    /// `LUT_BITS`/`LUT_SIZE` substituted for the macro parameter.
    macro_rules! define_canonical_decoder {
        ($name:ident, $lut_bits:expr) => {
            /// See `fixed_lut::CanonicalDecoder`'s doc comment (same design).
            pub(super) struct $name {
                ...
                end_id: [u64; MAX_CODE_LENGTH + 2],
                id_to_symbol: Vec<u32>,
                lut_symbol: [u32; 1 << $lut_bits],
                lut_length: [u8; 1 << $lut_bits],
            }
            ...
            impl $name {
                fn read(
                    ...
                    while code_index <= max_code_index {
                        ...
                        if code_len == LONG_ZEROCODE_RUN {
                            let zerun_bits =
                                read_bits(8, &mut code_bits, &mut code_bit_count, packed)?;
                            let zerun = usize::try_from(zerun_bits + SHORTEST_LONG_RUN).unwrap();
                            ...
                            if code_len > 0 {
                                count_per_length[u64_to_usize(code_len, "huffman code length")?] +=
                                    1;
                                symbol_lengths.push((u32::try_from(code_index)?, code_len as u8));
                            }
                            code_index += 1;
                ...
                fn build(
                    ...
                    let mut lut_symbol = [0_u32; 1 << $lut_bits];
                    let mut lut_length = [0_u8; 1 << $lut_bits];
                    for (prefix, (lut_symbol, lut_length)) in
                        lut_symbol.iter_mut().zip(lut_length.iter_mut()).enumerate()
                    {
                        let buffer = (prefix as u64) << (64 - $lut_bits);
                        ...
                        while remaining >= $lut_bits {
                            ...
                            if symbol == run_length_code {
                                let count = if remaining >= len + 8 {
                                    ...
                                    (read_word_at(&words, position + (64 - remaining) + len) >> 56)
                                        as usize
                                };
    ...
    define_canonical_decoder!(Decoder13, 13);
    define_canonical_decoder!(Decoder14, 14);

    pub(super) enum CanonicalDecoder {
        Bits13(Decoder13),
        Bits14(Decoder14),
    }
    ...
    impl CanonicalDecoder {
        pub(super) fn read(
            ...
            if use_14_bit_lut() {
                Decoder14::read(packed, min_code_index, max_code_index).map(Self::Bits14)
            } else {
                Decoder13::read(packed, min_code_index, max_code_index).map(Self::Bits13)
            }
        }
